...
class StateMachine:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.error_messages.append(["Cannot connect to the keithley"])
            self.state = "ERROR"
            return
        ...
        self.start_current = None # Lower limit
    ...
    def checkpressure(self):
        ...
        if ENABLE_MONODAQ:
            # create DCOM object
            print("Creating DCOM object.")
            dw = Dispatch("Dewesoft.App")

            # open Dewesoft
            print("Initialisation of DewesoftX ... ", end="")
            sys.stdout.flush()
            dw.Init()
            ...
            print("Loading of the configuration file ... ", end="")
            ...
            # build channel list
            print("Constructing the channels list.")
            ...
            while True:
                canal = 4
                nbr = 5
                #measure the voltage at pin 1
                m = 0
                for j in range(nbr):
                    conn = conn_list[canal]
                    if conn.DBDataSize >= 1:  # checks if channel 1 buffer can be read
                        BufPos = conn.DBPos
                ...
                if m > self.pressureLimits[1]:
                    print(f"pressure too low ({m:.0f} ohms)", end="\r")
                    consecutive = 0
                elif m < self.pressureLimits[0]:
                    print(f"pressure to high ({m:.0f} ohms)", end="\r")
                else:
                    print(f"pressure correct ({m:.0f} ohms)", end="\r")
                    consecutive += 1
    ...
    def defparametres(self):
        ...
        if self.previous_state == "CHECK PRESSURE":
            self.file_name = input("Savefile name : ")
        self.sample_name = input("Sample name : ")
        self.sample_thickness = float(input("Sample thickness (um) : "))
        self.stop_current = float(input("Maximum current (A): "))
        self.start_current = - self.stop_current
        self.nbr_mesures = int(input("Number of measure : "))
        self.step = abs(self.stop_current - self.start_current) / (self.nbr_mesures - 1)

        if input("\n\nAre the parameters corrects ? (y/n) : ") == "y":
            self.previous_state = self.state
            self.state = "MEASURE"
        else:
            self.previous_state = self.state
    ...
    def measure(self):
        ...
        print("Measured resistance: ", self.slope*4.532, "ohms")
        print(f"Résistivity : {self.slope*4.532*self.sample_thickness*1e-4} ohm/cm")
    ...
    def save(self):
        ...
        if input("Redo a measurment ? (y/n) : ") == "y":
            if input("Keep the same parameters ? (y/n) : ") == "y":
                self.previous_state = self.state
                self.state = "MEASURE"
            else:
                self.previous_state = self.state
